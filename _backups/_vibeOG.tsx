...
export default function VibeCreateScreen() {
  ...
  const saveVibe = async () => {
    // 1. Double check identity before firing
    const { data: { user } } = await supabase.auth.getUser();
    const effectiveEmail = user?.email || form.host_email;

    if (!effectiveEmail) {
      Alert.alert("Identity Error", "We couldn't verify who is throwing this line. Please try logging out and back in.");
      return;
    }
    ...
    try {
      // 2. Log for your dev build debugging
      console.log("Saving vibe for:", effectiveEmail);

      const { data, error } = await supabase.rpc("create_event_draft", {
        p_title: form.title.trim(),
        p_host_name: form.host_name.trim() || effectiveEmail.split('@')[0],
        p_host_email: effectiveEmail.toLowerCase().trim(),
