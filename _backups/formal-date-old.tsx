/**
 * Path: apps/mobile/app/create/formal-date.tsx
 * Description: Dedicated route for selecting a specific formal event date/time
 * and duration. Extracted from formal.tsx with minimal logic changes.
 */
...
export default function FormalDateScreen() {
  ...
  const openIOSPicker = (seedDate?: Date) => {
    setTempDate(seedDate ?? form.specificDate ?? new Date());
    setShowPicker(true);
  };

  const showAndroidPicker = () => {
    const baseDate = form.specificDate || new Date();
