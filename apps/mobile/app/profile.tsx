...
export default function ProfileScreen() {
  ...
  async function loadProfile() {
    ...
    try {
      ...
      const uniqueAttendedEventIds = Array.from(
        new Set(myRsvps.map((row) => row.event_id).filter(Boolean))
      );
      setAttendedCount(uniqueAttendedEventIds.length);

      let attendedActivity: ActivityItem[] = [];

      if (uniqueAttendedEventIds.length > 0) {
        const recentRsvps = myRsvps.slice(0, 10);
        const recentAttendedIds = Array.from(
          new Set(recentRsvps.map((row) => row.event_id).filter(Boolean))
        );

        if (recentAttendedIds.length > 0) {
          const { data: attendedEventsRes, error: attendedEventsError } = await supabase
            .from('events')
            .select('id, slug, title, starts_at, host_email, host_name')
            .in('id', recentAttendedIds);

          if (attendedEventsError) throw attendedEventsError;

          const attendedEventMap = new Map<string, EventRow>();
          (((attendedEventsRes as EventRow[]) || [])).forEach((event) => {
            attendedEventMap.set(event.id, event);
          });

          attendedActivity = recentRsvps
            .map((row) => {
              const event = attendedEventMap.get(row.event_id);
              if (!event) return null;

              return {
                id: `attended:${row.event_id}`,
      ...
      const merged = [...hostedActivity, ...attendedActivity]
        .sort((a, b) => {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.statCard}>
          <StyledText style={styles.statValue}>{attendedCount}</StyledText>
          <StyledText style={styles.statLabel}>Attended</StyledText>
        </View>
      </View>
            ...
            >
              <View style={styles.activityTopRow}>
                ...
                <View
                  style={[
                    styles.badge,
                    item.kind === 'hosted' ? styles.hostedBadge : styles.attendedBadge,
                  ]}
                >
                  <StyledText
                    style={[
                      styles.badgeText,
                      item.kind === 'hosted' ? styles.hostedBadgeText : styles.attendedBadgeText,
                    ]}
                  >
                    {item.kind === 'hosted' ? 'Hosted' : 'Attended'}
                  </StyledText>
                </View>
              </View>
              ...
              {item.kind === 'attended' && item.status ? (
