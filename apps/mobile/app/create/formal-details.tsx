...
const handleSavePress = () => {
  if (submitLockRef.current || loading || !canSave) return;
  ...
  setLoading(true);
    ...
    try {
      ...
      if (
        ...
        (form.whenMode === 'options' && form.pollOptions.length === 1)
      ) {
...
const isReachOut =
  ...
  (form.whenMode === 'options' && form.pollOptions.length === 0);
...
const result = await createVibeDraft({
  ...
  keyword: `evt-${Math.random().toString(36).substring(2, 7)}`,
  gifKey: 'waves',
  eventType,
  proposedDates,
          ...
          <View>
            ...
            <View style={styles.nav}>
              ...
              <TouchableOpacity
                ...
                disabled={loading || !canSave || submitLockRef.current}
              >
                {loading ? (
